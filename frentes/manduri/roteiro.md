# Manduri/SP — Roteiro do teaser institucional (v2)

> **v2 — registro arbitrado por Victor em 2026-07-27.**
> A peça é um **TEASER institucional cinematográfico de presença serena**.
> Não é ocorrência e não é peça explicativa.
>
> Formato: **39,5s** · 8 planos visuais + cartão final · master 16:9 2K · animação 1080p.
> Shot list operativa: [`manifesto-producao.md`](manifesto-producao.md) (M01–M08).
> Montagem: [`edl.md`](edl.md).
>
> ⚠️ **Duração revisada de 45s → 39,5s**: M07 deixou de ser 2 planos de 5s e passou a
> ser **um clipe de 4,5s** (reuso de Avaré com montagem interna). Segue dentro da faixa
> de teaser 30–60s.

## O que mudou da v1 para a v2

A v1 era um filme de **ocorrência** (caso grave, travessia, estabilização, passagem de
bastão). Victor arbitrou para **presença serena**. Consequência direta:

| v1 (descartado) | v2 (vigente) |
|---|---|
| B03 Pronto Municipal como "falta" dramática | M05 — equipe em rotina tranquila, sem paciente |
| B07 interior, equipe estabilizando paciente | **Removido.** Não há paciente na peça |
| B08 chegada e passagem de bastão | **Removido.** Não há chegada |

**O eixo da transferência sobrevive — mas só em locução mínima, nunca em imagem.**
A cidade tranquila é o que se vê; a narração diz, sem dramatizar, o que ela depende.

## Travas da peça

| Trava | Razão |
|---|---|
| ❌ **Sem ocorrência, paciente ou acidente** | Registro de teaser arbitrado por Victor |
| ❌ **Sem sirene e sem giroflex** | Idem. Luzes de emergência **apagadas o filme inteiro** |
| ❌ **Sem motolância** | Fora do escopo comercial desde jul/2026 (decisão do CEO) |
| ❌ **Sem menção a consórcio, rateio ou governança** | Arranjo não assinado; filme é peça pública |
| ❌ **Sem alusão a acidente real** | Há caso fatal real na rota. Dignidade, sem exceção |
| ✅ **Padrão FRIO** | Audiência externa: neutro, factual, sem advocacy |
| ✅ **Cidade pequena de verdade** | Ruas estreitas, sem feição de capital |

## Linguagem de câmera — aprovada na v2

Dinamismo **com elegância**, não urgência:

- **Tracking lateral baixo** acompanhando a USB
- **Leve dolly / push-in**
- **Parallax** de árvores e postes em primeiro plano
- Movimento **fluido de gimbal/FPV**

❌ Câmera nervosa · velocidade de emergência · whip-pan · shake.

> Nota: o DNA original de Avaré ("agitado, FPV, muita ação") **foi temperado**. O
> anamórfico 35mm, o motion blur e a profundidade cinematográfica permanecem; a
> agitação, não.

## Dado-âncora — PENDENTE

O único número que a locução carrega é o **tempo rodoviário de Manduri até a retaguarda
em Avaré**, e ele ainda não existe. Aparece como `[TEMPO A CONFIRMAR]`.
**Não inventar, não estimar, não arredondar.**

Liberado: **9.871 habitantes** (✅ IBGE, Censo 2022).
Proibido: preço, cota, margem, abrangência da CRU.

---

## Estrutura

### ATO 1 — A cidade · *luz: golden hour*

| Plano | Tempo | Conteúdo |
|---|---|---|
| **M01** | 0:00–0:05 | Portal da cidade. USB entra em velocidade normal. Tracking lateral baixo |
| **M02** | 0:05–0:10 | Prefeitura / Rua Bahia ao fundo. USB passa. Leve push-in |
| **M03** | 0:10–0:15 | Igreja Matriz / Praça Getúlio Vargas. USB atravessa. Parallax das árvores |
| **M04** | 0:15–0:20 | Rua residencial. Presença cotidiana |
| **M05** | 0:20–0:25 | Pronto Municipal. USB parada, equipe em rotina. Sem maca, sem paciente |

### ATO 2 — A travessia · *luz: hora azul (troca no corte, nunca dentro do plano)*

| Plano | Tempo | Conteúdo |
|---|---|---|
| **M06** | 0:25–0:30 | Saída para a rodovia. USB se afastando, traseira para a câmera |
| **M07** | 0:30–0:34,5 | CRU regional ♻️ — médico regulador (close) + plano geral da central. **4,5s**, reuso de Avaré |

> **O TARM saiu da peça.** O clipe de atendimento telefônico de Avaré foi descartado por
> exibir telefone fictício com DDD 11 e interface legível demais — ruído factual. A
> regulação é representada só pelo regulador e pela central. Ver `edl.md`.

### ATO 3 — Assinatura

| Plano | Tempo | Conteúdo |
|---|---|---|
| **M08** | 0:34,5–0:39,5 | Navy-black `#04060C`, logo branco, fio dourado, "Gestão em Saúde". **100% pós**, só marca Samais |

---

## Locução (mínima — 9 planos, 4 falas)

> *M02* — Manduri tem nove mil oitocentos e setenta e uma pessoas.
> *M05* — Tem pronto atendimento. Não tem hospital.
> *M06* — O caso grave atravessa `[TEMPO A CONFIRMAR]` até a retaguarda.
> *M07* — **A regulação acontece na central regional.**
> *M08* — Samais. Gestão em saúde.

> ✅ **Locução de M07 fechada por Victor.** Substitui *"Quem regula esse tempo não está
> em Manduri. Está na central regional."* — mais clara e factual, e resolve o aperto de
> tempo: **6 palavras (~2,3s) num plano de 4,5s**, deixando **~2,2s de pausa** antes da
> assinatura.

**~34 palavras** em 39,5s — deliberadamente abaixo do limite: teaser respira, não explica.
A tese "a ambulância é o tratamento" **não é dita**. Fica implícita na sequência
M05 → M06 → M07. Se Victor quiser explicitá-la, entra em M06 — mas custa o tom.

## Cláusulas obrigatórias

**Cinematografia** (todo prompt de frame):
```
subtle anamorphic flare on the light source, FEW dust particles floating in the light,
light atmospheric haze, cinematic depth of field, rich micro-contrast
```

**Movimento** (todo prompt de animação):
```
Low cinematic tracking camera moves smoothly with the vehicle, subtle forward dolly
and natural foreground-tree parallax, keeping the background stable and recognizable.
Ordinary traffic speed, wheels rotate naturally, suspension remains grounded.
All emergency lights and beacons stay OFF for the entire shot. Quiet teaser mood,
realistic motion blur, no urgency, no siren, no camera shake, no architecture
morphing, no vehicle deformation.
```

**Uniforme** (todo prompt com pessoa):
```
navy SAMU coverall #1C2D4A, WHITE reflective bands, orange side piping,
red Star of Life, "SAMU 192" in orange, Brazilian flag patch
```
❌ Nunca: farda branca · faixa amarelo hi-vis · faixa vermelha no socorrista · EMS
americano/europeu.

## Reaproveitamento de Avaré

♻️ **M07** (CRU — mesma central real). Todo o resto é produção nova.
Os blocos de motolância de Avaré **não servem**.

## Pendências

1. **CNES** — Pronto Municipal 24h e ausência de hospital (trava M05 e a locução).
2. **Tempo rodoviário Manduri→Avaré** (trava a locução de M06).
3. **Referências reais de locação** — portal, Prefeitura/Rua Bahia, rodovia, Pronto
   Municipal. Igreja Matriz ✅ já tem master 2K aprovado. Nunca inventar locação.
